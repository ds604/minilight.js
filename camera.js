...
function Camera(stream) {
  ...
  var up, right = normalize(cross(Vector3(0, 1, 0), viewDirection))
  if (!isZero(right))
    ...
    getFrame: function(scene, image, random) {
      ...
      for (var y = 0; y < image.height; ++y) {
        for (var x = 0; x < image.width; ++x) {
          ...
          var offset = add(scale(xCoeff, right), scale(yCoeff * aspect, up))
          var sampleDirection =
          normalize(add(viewDirection, scale(Math.tan(viewAngle * 0.5), offset)))
          var radiance = raytracer.getRadiance(viewPosition, sampleDirection, random, null)
